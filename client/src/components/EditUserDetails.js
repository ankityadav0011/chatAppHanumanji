...
// old code 
    ...
    //     e.stopPropagation()
    //     try {
    //         const URL = `http://localhost:5009/api/updateUser`
    ...
    const handleSubmit = async (e) => {
        ...
        e.stopPropagation();
        try {
            const URL = `${REACT_APP_BACKEND_URL}/api/updateUser`;
        

    
            // Clean data to avoid non-serializable values
            const cleanData = {
                name: data.name,
            ...
            console.error(error);
            taost.error('An error occurred while updating profile details.');
        }
    };
